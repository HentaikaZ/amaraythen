...
function main()
   ...
   -- while not sampIsLocalPlayerSpawned() do wait(200) end
    if not doesDirectoryExist('moonloader/config/Amaraythen') then createDirectory('moonloader/config/Amaraythen') end
    if not doesFileExist(getWorkingDirectory()..'/config/Amaraythen/Amaraythen.ini') then inicfg.save(cfg, 'Amaraythen/Amaraythen.ini') end
    if not doesFileExist(getWorkingDirectory()..'/config/Amaraythen/bell.wav') then
        downloadUrlToFile('https://github.com/Venibon/Ghetto-Helper/raw/main/bell.wav', getWorkingDirectory()..'/config/Amaraythen/bell.wav')
    end
    if not doesFileExist(getWorkingDirectory()..'/resource/fonts/fontawesome-webfont.tt') then 
        ...
        sampRegisterChatCommand("cho", function()
            ogrove,oballas,orifa,ovagos,oaztec = 0,0,0,0,0
            for l = 0, 1004 do
